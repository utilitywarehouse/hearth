...
Pre-built action rows. Use when you need a consistent list of labelled actions.

**CardActionLink / CardActionButton props:**

| Prop | Values | Notes |
|---|---|---|
| `heading` | string | Main action label |
| `helperText` | string | Secondary line below heading |
| `leadingIcon` | React node | Icon on the left |
| `leadingIconContainerColorScheme` | service color string | Background colour of the icon container |
| `trailingIcon` | React node | Icon on the right |
| `badge` | React node | Badge element |
| `badgePlacement` | `'middle'` \| `'bottom'` \| `'right'` | |
| `href` | string | CardActionLink only |
| `target` | string | CardActionLink only |
| `onClick` | function | CardActionButton only |

```tsx
// Single pre-built action
<Card>
  <CardActionLink
    heading="Electricity"
    helperText="Last reading: 30 Oct"
...
// Multiple actions with responsive layout
<Card>
...
<Card>
  ...
  <CardBannerContent
    ...
    description="Get up to £150 in credit when you join"
  />
</Card>
```

**CardBannerContent props:** `heading`, `description`, `textAlign`, `alignItems`
...
- For cards that are entirely clickable, a single `CardInteraction asChild` wrapping a `<Link>` is correct
- Multiple `CardInteraction` elements are supported (one primary + one secondary)
- Use `aria-labelledby` pointing to the card heading when extra context helps screen readers

**Gotcha:** `paddingNone` is needed when using `IconContainer fill="height"` — the icon container must reach the card edges to look correct.
