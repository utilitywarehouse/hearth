...
Use the `Spinner` component if you need to indicate a loading state withing the `Modal`
...
| `defaultOpen`  | `boolean`                 | Whether the modal is open by default                                      |
| `onOpenChange` | `(open: boolean) => void` | Callback that fires when the open state changes                           |
| `modal`        | `boolean`                 | Whether the modal is modal (blocks interaction with the rest of the page) |
