...
- `current` represents a completed step in the journey - The card is collapsed
...
- `previous` represents the active step the user is currently completing - The
...
Each `CardAccordion` component requires a `value` to be able to automatically
...
`CardActionButton` component will be either a `next` or `previous` action, and
can render a custom label if necessary.

```tsx
<CardAccordionFooter>
  <CardActionButton action="previous">Back to 1a</CardActionButton>
