/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react-vite';
import { FingerprintMediumIcon } from '@utilitywarehouse/hearth-react-icons/lib/FingerprintMediumIcon';

const meta: Meta<typeof FingerprintMediumIcon> = {
  title: 'Individual Icons/FingerprintMediumIcon',
  component: FingerprintMediumIcon,
};

export default meta;
type Story = StoryObj<typeof FingerprintMediumIcon>;

export const FingerprintMediumIconStory: Story = {
  name: 'FingerprintMediumIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
