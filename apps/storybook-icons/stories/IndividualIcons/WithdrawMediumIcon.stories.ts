/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react';
import { WithdrawMediumIcon } from '@utilitywarehouse/hearth-react-icons/lib/WithdrawMediumIcon';

const meta: Meta<typeof WithdrawMediumIcon> = {
  title: 'Individual Icons/WithdrawMediumIcon',
  component: WithdrawMediumIcon,
};

export default meta;
type Story = StoryObj<typeof WithdrawMediumIcon>;

export const WithdrawMediumIconStory: Story = {
  name: 'WithdrawMediumIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
