/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react';
import { BookmarkMediumIcon } from '@utilitywarehouse/hearth-react-icons/lib/BookmarkMediumIcon';

const meta: Meta<typeof BookmarkMediumIcon> = {
  title: 'Individual Icons/BookmarkMediumIcon',
  component: BookmarkMediumIcon,
};

export default meta;
type Story = StoryObj<typeof BookmarkMediumIcon>;

export const BookmarkMediumIconStory: Story = {
  name: 'BookmarkMediumIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
