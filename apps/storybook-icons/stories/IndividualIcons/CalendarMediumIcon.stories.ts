/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react';
import { CalendarMediumIcon } from '@utilitywarehouse/hearth-react-icons/lib/CalendarMediumIcon';

const meta: Meta<typeof CalendarMediumIcon> = {
  title: 'Individual Icons/CalendarMediumIcon',
  component: CalendarMediumIcon,
};

export default meta;
type Story = StoryObj<typeof CalendarMediumIcon>;

export const CalendarMediumIconStory: Story = {
  name: 'CalendarMediumIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
