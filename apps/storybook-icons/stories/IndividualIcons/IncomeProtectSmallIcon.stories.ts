/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react';
import { IncomeProtectSmallIcon } from '@utilitywarehouse/hearth-react-icons/lib/IncomeProtectSmallIcon';

const meta: Meta<typeof IncomeProtectSmallIcon> = {
  title: 'Individual Icons/IncomeProtectSmallIcon',
  component: IncomeProtectSmallIcon,
};

export default meta;
type Story = StoryObj<typeof IncomeProtectSmallIcon>;

export const IncomeProtectSmallIconStory: Story = {
  name: 'IncomeProtectSmallIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
