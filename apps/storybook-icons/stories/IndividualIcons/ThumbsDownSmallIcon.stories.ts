/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react';
import { ThumbsDownSmallIcon } from '@utilitywarehouse/hearth-react-icons/lib/ThumbsDownSmallIcon';

const meta: Meta<typeof ThumbsDownSmallIcon> = {
  title: 'Individual Icons/ThumbsDownSmallIcon',
  component: ThumbsDownSmallIcon,
};

export default meta;
type Story = StoryObj<typeof ThumbsDownSmallIcon>;

export const ThumbsDownSmallIconStory: Story = {
  name: 'ThumbsDownSmallIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
