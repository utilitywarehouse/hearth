/* Don't edit this file directly, it was auto generated */
import type { Meta, StoryObj } from '@storybook/react-vite';
import { TorchOffMediumIcon } from '@utilitywarehouse/hearth-react-icons/lib/TorchOffMediumIcon';

const meta: Meta<typeof TorchOffMediumIcon> = {
  title: 'Individual Icons/TorchOffMediumIcon',
  component: TorchOffMediumIcon,
};

export default meta;
type Story = StoryObj<typeof TorchOffMediumIcon>;

export const TorchOffMediumIconStory: Story = {
  name: 'TorchOffMediumIcon',
  argTypes: {
    children: { table: { disable: true } },
    color: { table: { disable: true } },
  },
};
