export declare const banner: {
  readonly default: {
    readonly content: {
      readonly gap: 'var(--h-banner-default-content-gap)';
      readonly textGap: 'var(--h-banner-default-content-text-gap)';
    };
    readonly gap: 'var(--h-banner-default-gap)';
    readonly horizontal: {
      readonly illustrationWidth: 'var(--h-banner-default-horizontal-illustration-width)';
      readonly imageMaxWidth: 'var(--h-banner-default-horizontal-image-max-width)';
      readonly imageMinWidth: 'var(--h-banner-default-horizontal-image-min-width)';
      readonly maxWidth: 'var(--h-banner-default-horizontal-max-width)';
      readonly minWidth: 'var(--h-banner-default-horizontal-min-width)';
    };
    readonly padding: 'var(--h-banner-default-padding)';
    readonly vertical: {
      readonly imageHeight: 'var(--h-banner-default-vertical-image-height)';
      readonly maxWidth: 'var(--h-banner-default-vertical-max-width)';
      readonly minWidth: 'var(--h-banner-default-vertical-min-width)';
    };
  };
  readonly highlight: {
    readonly content: {
      readonly gap: 'var(--h-banner-highlight-content-gap)';
    };
    readonly gap: 'var(--h-banner-highlight-gap)';
    readonly heading: {
      readonly broadbandBlueBackgroundColor: 'var(--h-banner-highlight-heading-broadband-blue-background-color)';
      readonly cashbackLilacBackgroundColor: 'var(--h-banner-highlight-heading-cashback-lilac-background-color)';
      readonly energyBlueBackgroundColor: 'var(--h-banner-highlight-heading-energy-blue-background-color)';
      readonly insuranceOrangeBackgroundColor: 'var(--h-banner-highlight-heading-insurance-orange-background-color)';
      readonly mobileRoseBackgroundColor: 'var(--h-banner-highlight-heading-mobile-rose-background-color)';
      readonly piggyPinkBackgroundColor: 'var(--h-banner-highlight-heading-piggy-pink-background-color)';
      readonly yellowBackgroundColor: 'var(--h-banner-highlight-heading-yellow-background-color)';
    };
    readonly padding: 'var(--h-banner-highlight-padding)';
  };
};
