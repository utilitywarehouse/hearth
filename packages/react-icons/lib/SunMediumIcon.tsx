import { forwardRef } from 'react';
import { IconProps } from './types';
export const SunMediumIcon = forwardRef<SVGSVGElement, IconProps>(({
  color = 'currentColor',
  title,
  titleId,
  ...props
}, ref) => {
  return <svg xmlns="http://www.w3.org/2000/svg" width={24} height={24} fill="none" viewBox="0 0 24 24" aria-hidden={!title} focusable="false" role="img" ref={ref} aria-labelledby={titleId} {...props}>{title ? <title id={titleId}>{title}</title> : null}<path fill={color} d="M11 5V1h2v4zm6.65 2.75-1.375-1.375 2.8-2.875 1.4 1.425zM19 13v-2h4v2zm-8 10v-4h2v4zM6.35 7.7 3.5 4.925l1.425-1.4L7.75 6.35zm12.7 12.8-2.775-2.875 1.35-1.35 2.85 2.75zM1 13v-2h4v2zm3.925 7.5-1.4-1.425 2.8-2.8.725.675.725.7zM12 18q-2.5 0-4.25-1.75T6 12t1.75-4.25T12 6t4.25 1.75T18 12t-1.75 4.25T12 18" /></svg>;
});
SunMediumIcon.displayName = "SunMediumIcon";