...
export type FieldsetProps = ComponentPropsWithout<'fieldset', RemovedProps> &
  ...
  (
    ...
    | {
        // If the label prop is not being used, presumably because a separate label
        // is being rendered, then these other props should also not be used, otherwise
        // they could be rendered before a child legend, breaking the associating
        // between the fieldset and the legend
        label?: never;
        helperText?: never;
        validationText?: never;
