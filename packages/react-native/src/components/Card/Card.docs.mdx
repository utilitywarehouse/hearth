...
- **Explicit wrapper marker**: Wrapper components must set `isCardActionWrapper = true` to be treated as actions
...
1. **Always use a wrapper component** for better code organisation and mark it as an action wrapper:
...
ActionItem.isCardActionWrapper = true;
