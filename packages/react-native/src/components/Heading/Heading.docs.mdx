...
`Heading` automatically sets `accessibilityRole="header"`, so VoiceOver and TalkBack can announce it as a heading without any extra configuration.

- Use `Heading` for actual screen or section titles so assistive technologies can expose the right document structure.
- If you need styled text without heading semantics, use one of the text components instead.
