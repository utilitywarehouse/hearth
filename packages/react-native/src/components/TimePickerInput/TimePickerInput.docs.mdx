...
`TimePickerInput` inherits all React Native `TextInput` props (except `children`) and adds the following:
...
`TimePickerInput` doesn't display validation styles by itself. Pair it with `FormField` (or your own state) to surface errors whenever the typed value doesn't match your expected format.
