...
| `labels`       | `Partial<Record<RatingValue, string>>` | Override labels for specific rating values. For the emoji variant, `labels[1]` and `labels[5]` override the start and end labels. | `undefined` |
...
Use `variant="emojis"` to show emoji faces instead of stars. Labels default to "Very dissatisfied" and "Very satisfied" at the extremes. Override them with `labels={{ 1: 'Custom start', 5: 'Custom end' }}`.
